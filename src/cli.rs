...
impl Args {
    ...
    pub fn resolve_test_inputs(&self) -> Result<Vec<ResolvedTest>> {
        ...
        for test in resolved_tests {
            if !unique_tests.iter().any(|t: &ResolvedTest| t.path == test.path) {
    ...
    fn resolve_directory(&self, dir_path: &Path) -> Result<Vec<ResolvedTest>> {
        ...
        if tests.is_empty() {
            return Err(anyhow!("No .test files found in directory: {}", dir_path.display()));
        }
    ...
    fn resolve_file(&self, file_path: &Path) -> Result<Vec<ResolvedTest>> {
        if !file_path.extension().map_or(false, |ext| ext == "test") {
            return Err(anyhow!("File must have .test extension: {}", file_path.display()));
        }
    ...
    fn resolve_test_name_or_partial_path(&self, input: &str) -> Result<Vec<ResolvedTest>> {
        ...
        Err(anyhow!(
            ...
            input, clean_input, with_extension
        ))
    }
    ...
    fn path_to_test_name(&self, path: &Path, base_dir: Option<&Path>) -> Result<String> {
        ...
        let file_name = path_without_ext
            .file_name()
            .unwrap_or_default();
...
mod tests {
    ...
    fn test_resolve_directory() {
        ...
        fs::write(feature_dir.join("advanced.test"), "# Advanced test\nSELECT 2;").unwrap();
        fs::write(feature_dir.join("complex.test"), "# Complex test\nSELECT 3;").unwrap();
