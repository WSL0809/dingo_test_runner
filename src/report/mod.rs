...
pub fn create_renderer_with_allure_dir(
    format: &str,
    allure_dir: Option<&str>,
) -> Result<Box<dyn ReportRenderer>> {
    match format.to_lowercase().as_str() {
        "plain" | "text" => Ok(Box::new(PlainTextRenderer)),
        "html" => Ok(Box::new(SimpleHtmlRenderer)),
        "xunit" | "xml" | "junit" => Ok(Box::new(XunitRenderer)),
        ...
        "allure" => {
            if let Some(dir) = allure_dir {
                ...
                Err(anyhow::anyhow!(
                    "Allure format requires --allure-dir parameter"
                ))
            }
        }
        _ => Err(anyhow::anyhow!("Unsupported report format: {}", format)),
...
impl ReportRenderer for SimpleHtmlRenderer {
    fn render(&self, suite: &TestSuiteResult) -> Result<String> {
        ...
        html.push_str(&format!(
            "    <p><strong>总测试数:</strong> {}</p>\n",
            suite.total_tests()
        ));
        html.push_str(&format!(
            "    <p><strong>通过:</strong> <span class=\"passed\">{}</span></p>\n",
            suite.passed_tests()
        ));
        html.push_str(&format!(
            "    <p><strong>失败:</strong> <span class=\"failed\">{}</span></p>\n",
            suite.failed_tests()
        ));
        html.push_str(&format!(
            "    <p><strong>跳过:</strong> <span class=\"skipped\">{}</span></p>\n",
            suite.skipped_tests()
        ));
        html.push_str(&format!(
            "    <p><strong>通过率:</strong> {:.1}%</p>\n",
            suite.pass_rate()
        ));
        html.push_str(&format!(
            "    <p><strong>总用时:</strong> {:.2}s</p>\n",
        ...
        if !suite.cases.is_empty() {
            ...
            for (index, case) in suite.cases.iter().enumerate() {
                ...
                let row_class = if case.status == TestStatus::Failed {
                    " class=\"error\""
                } else {
                    ""
                };
                ...
                html.push_str(&format!(
                    ...
                    status_class,
                    status_symbol,
                    status_class.to_uppercase()
                ));
                html.push_str(&format!("        <td>{}</td>\n", case.duration_ms));
                html.push_str(&format!("        <td>{}</td>\n", case.passed_queries));
                html.push_str(&format!("        <td>{}</td>\n", case.failed_queries));

                let first_error = case
                    .errors
                    .first()
                    .map(|e| html_escape(e))
...
impl ReportRenderer for XunitRenderer {
    fn render(&self, suite: &TestSuiteResult) -> Result<String> {
        ...
        xml.push_str(&format!(
            ...
            xunit::escape_xml(&suite.environment.os)
        ));
        xml.push('\n');
        xml.push_str(&format!(
            r#"    <property name="rust_version" value="{}"/>"#,
            xunit::escape_xml(&suite.environment.rust_version)
        ));
        xml.push('\n');
        ...
        if let Some(ref mysql_version) = suite.environment.mysql_version {
            xml.push_str(&format!(
                ...
                xunit::escape_xml(mysql_version)
            ));
            xml.push('\n');
        }
