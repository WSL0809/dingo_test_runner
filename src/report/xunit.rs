...
pub fn write_xunit_report(suite: &TestSuiteResult, file_path: &str) -> Result<()> {
    ...
    writeln!(file, r#"<testsuite name="{}" tests="{}" failures="{}" skipped="{}" time="{:.3}" timestamp="{}">"#,
...
fn write_properties(file: &mut File, suite: &TestSuiteResult) -> Result<()> {
    ...
    writeln!(file, r#"    <property name="os" value="{}"/>"#, escape_xml(&suite.environment.os))?;
    writeln!(file, r#"    <property name="rust_version" value="{}"/>"#, escape_xml(&suite.environment.rust_version))?;
    
    if let Some(ref git_commit) = suite.environment.git_commit {
        writeln!(file, r#"    <property name="git_commit" value="{}"/>"#, escape_xml(git_commit))?;
    }
    
    if let Some(ref mysql_version) = suite.environment.mysql_version {
        writeln!(file, r#"    <property name="mysql_version" value="{}"/>"#, escape_xml(mysql_version))?;
    }
    
    let cli_args = suite.environment.cli_args.join(" ");
    writeln!(file, r#"    <property name="cli_args" value="{}"/>"#, escape_xml(&cli_args))?;
...
fn write_test_case(file: &mut File, case: &crate::tester::tester::TestResult) -> Result<()> {
    match case.status {
        ...
        TestStatus::Failed => {
            ...
            writeln!(file, r#"    <failure message="Test failed" type="TestFailure">"#)?;
            ...
            if !case.stdout.is_empty() {
                writeln!(file, "    <system-out><![CDATA[{}]]></system-out>", case.stdout)?;
            }
            
            if !case.stderr.is_empty() {
                writeln!(file, "    <system-err><![CDATA[{}]]></system-err>", case.stderr)?;
            }
